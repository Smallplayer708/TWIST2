...
class XRobotTeleopToRobot:
    ...
    def process_retargeting(self, smplx_data, headset_data=None):
        ...
        qpos = self.retarget.retarget(smplx_data, offset_to_ground=True, root_yaw=root_yaw).copy()
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=float,
        default=5e-1,
        help="GMR IK damping. Default 5e-1 (0.5, smooth but sluggish); "
             "1e-1 or 1e-2 = faster/sharper leg tracking (used by tuned mode).",
    )
    parser.add_argument(
        "--yaw_gain",
        type=float,
